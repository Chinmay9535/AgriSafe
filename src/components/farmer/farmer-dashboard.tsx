...
interface Batch {
  ...
  status: 'PENDING' | 'VERIFIED' | 'REJECTED' | 'ACTIVE' | 'BLOCKCHAIN_SYNCED';
  verified?: boolean;
...
export function FarmerDashboard(): React.JSX.Element {
  ...
  const [certificateBatch, setCertificateBatch] = useState<Batch | null>(null);
  ...
  const uploadFilesToSupabase = async (files: File[], stageName: string): Promise<string[]> => {
  ...
  const handleSubmitStage = async (stageIndex: number) => {
    ...
    try {
      ...
      toast.info(`Uploading ${stage.images.length} images for "${stage.name}"...`);
      const uploadedImageUrls = await uploadFilesToSupabase(stage.images, stage.name);

      // Submit stage with Supabase URLs
      const stageWithImages = {
        ...stage,
        images: uploadedImageUrls // Use Supabase URLs instead of base64
      };
      ...
      const response = await fetch('/api/stages', {
        ...
        body: JSON.stringify({
          ...
          farmerId: user.id,
          stages: [stageWithImages] // Submit only this stage
        }),
      });

      if (response.ok) {
        toast.success(`Stage "${stage.name}" submitted successfully!`);
        ...
        const errorData = await response.json();
        toast.error(errorData.error || `Failed to submit stage "${stage.name}"`);
      }
    } catch (error) {
      ...
      if (error instanceof Error) {
        ...
        toast.error(`Failed to submit stage "${stage.name}"`);
      }
    } finally {
      setIsStageLoading(false);
    }
  ...
  const handleDownloadCertificate = async (batch: Batch) => {
    try {
      ...
      if (!batch.verified || !batch.blockchainTxHash) {
        toast.error('Certificate is only available for blockchain-verified batches');
        return;
      }
  ...
  return (
    <div className="container mx-auto px-4 py-6 space-y-6">
                ...
                <div key={batch.id || index} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      {/* Show QR Code for Verified Batches */}
                      {batch.verificationStatus === 'VERIFIED' && batch.qrCode && (
                        <div className="mt-4 p-4 bg-gradient-to-br from-green-50 to-blue-50 rounded-lg border-2 border-green-300 shadow-sm">
                          <div className="flex items-center gap-4">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-gray-700 mb-2">
                                Scan this code to view complete batch details and blockchain verification
                              </p>
                              {batch.blockchainTxHash && (
                    ...
                    <div className="ml-4 flex gap-2">
                      ...
                      {batch.verified && batch.blockchainTxHash && (
                        <Button
                          variant="outline"
                          size="sm"
        ...
        <DialogContent className="sm:max-w-3xl bg-white rounded-2xl border-0 shadow-2xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedBatch && (
            <div className="space-y-6 py-4">
              ...
              {selectedBatch.verified && selectedBatch.blockchainTxHash && (
                  ...
                  <div className="bg-gradient-to-r from-amber-50 via-yellow-50 to-amber-50 rounded-xl p-6 border-2 border-amber-200">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="font-bold text-lg text-amber-900 mb-1">🎉 Blockchain Certificate Available</h4>
                          <p className="text-sm text-amber-700 mb-1">
                            Congratulations! Your batch has been verified and permanently recorded on the blockchain.
                          </p>
                          <p className="text-xs text-amber-600 font-semibold">
                            ✓ Admin verified | ✓ Blockchain synced | ✓ QR code generated
                          </p>
                        </div>
                      </div>
                      <Button
      ...
      {/* Certificate Modal */}
      {showCertificate && certificateBatch && (
        <CertificateModal
          batch={{
            id: certificateBatch.id,
            batchCode: certificateBatch.batchCode || '',
            name: certificateBatch.name,
            cropType: certificateBatch.cropType || certificateBatch.category,
            quantity: certificateBatch.quantity || 0,
            farmerName: certificateBatch.farmerName || user?.name || 'Unknown Farmer',
            location: typeof certificateBatch.location === 'string' 
              ? certificateBatch.location 
              : certificateBatch.location?.address || 'Unknown Location',
            harvestDate: certificateBatch.harvestDate || certificateBatch.actualHarvestDate?.toString() || new Date().toISOString(),
            qrCode: certificateBatch.qrCode,
            verifyUrl: certificateBatch.verifyUrl,
            blockchainTxHash: certificateBatch.blockchainTxHash,
            blockchainNetwork: certificateBatch.blockchainNetwork,
            blockchainSyncedAt: certificateBatch.blockchainSyncedAt,
          }}
          onClose={() => {
