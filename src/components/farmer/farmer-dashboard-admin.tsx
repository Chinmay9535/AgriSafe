    ...
    <>
      <div className={`relative cursor-pointer ${className}`} onClick={() => setShowImageModal(true)}>
        ...
        {isSupabaseImage && (
          <Badge 
            variant="outline" 
            className="absolute top-2 left-2 text-xs font-semibold bg-green-100 text-green-800 border-green-300"
          >
            ✓ Real Image
          </Badge>
        )}
...
export function FarmerDashboardAdmin(): React.JSX.Element {
  ...
  const pendingBatches = allBatches.filter(batch => 
    ...
    (!batch.verified && batch.verificationStatus !== 'VERIFIED' && batch.verificationStatus !== 'REJECTED')
  ).length;
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 px-3 py-1 font-semibold">
                          {batches.filter(b => b.verificationStatus === 'VERIFIED' || b.verified === true).length} verified
                        </Badge>
                        <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200 px-3 py-1 font-semibold">
                          {batches.filter(b => b.verificationStatus === 'REJECTED' || b.verified === false).length} rejected
                        </Badge>
                        <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200 px-3 py-1 font-semibold">
                          {batches.filter(b => b.verificationStatus === 'PENDING' || (!b.verified && b.verificationStatus !== 'VERIFIED' && b.verificationStatus !== 'REJECTED')).length} pending
                        </Badge>
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  {/* QR Code Section */}
                  {selectedBatch.batchCode === 'FB003' && (
                    <div>
                      <h4 className="font-semibold mb-3 text-lg">Batch QR Code</h4>
                        ...
                        <CardContent className="p-4">
                          ...
                          <div className="text-center space-y-2">
                            <p className="text-sm text-gray-600 font-mono break-all bg-gray-50 p-2 rounded">
                              {selectedBatch.batchCode || selectedBatch.id}
                            </p>
                            <Button
                              onClick={() => {
                                const link = document.createElement('a');
                                link.download = `${selectedBatch.batchCode}_QR.jpg`;
                                link.href = '/fb003.jpg';
